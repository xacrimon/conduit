...
pub async fn get_user_pastes(db: impl IntoTarget<'_>, user_id: UserId) -> Result<Vec<PasteInfo>> {
    atomic(db, (), |txn, _| {
        async move {
...
pub async fn get_paste(db: impl IntoTarget<'_>, paste_id: &str) -> Result<Option<PasteWithFile>> {
    atomic(db, paste_id, |txn, paste_id| {
        async move {
...
pub async fn delete_paste(db: impl IntoTarget<'_>, user_id: UserId, paste_id: &str) -> Result<()> {
    atomic(db, paste_id, |txn, paste_id| {
        async move {
            sqlx::query!(
